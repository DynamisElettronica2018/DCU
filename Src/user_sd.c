...
extern inline void SD_Open_File(void)
{
  ...
  else
  {
    ...
    SD_Write_Len(header_Packet_Buffer);
...
extern inline void SD_Close_And_Open_File(void)
{
  if(f_close(&SDFile) != FR_OK)
  { 
    dcu_State_Packet[DCU_STATE_PACKET_ACQUISITION_ON] = UDP_DCU_STATE_ERROR;
  ...
  if(f_open(&SDFile, SD_Filename, FA_OPEN_APPEND | FA_WRITE) != FR_OK)
  {
    UDP_Send_Error(OPEN_FILE_ERROR);
    dcu_State_Packet[DCU_STATE_PACKET_ACQUISITION_ON] = UDP_DCU_STATE_ERROR;
    HAL_GPIO_WritePin(LED_RED_GPIO_Port, LED_RED_Pin, GPIO_PIN_RESET);
...
static inline void SD_Get_File_Name(char *filename)
{
